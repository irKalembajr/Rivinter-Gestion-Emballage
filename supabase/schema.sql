...
begin
  if not public.is_principal_admin() then
    raise exception 'Réinitialisation réservée à l administrateur principal';
  end if;
  ...
  if p_restore_seed then
    ...
    set value = jsonb_build_object('locked', false, 'locked_at', null, 'locked_by', null)
    where key = 'initial_stock';
    perform public.seed_initial_stocks();
  end if;

  insert into public.app_events (actor_id, action, details)
  values (auth.uid(), 'reset_company_data', jsonb_build_object('restore_seed', p_restore_seed));
end;
$$;
